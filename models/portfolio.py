...
class Portfolio:
    ...
    def print_sell(self, date, data, info_positions, result):
        print(f"{date} | SELL ({result}) | {data['symbol']} x{info_positions['total_nb']} | Capital: {round(self.capital, 2)}$ | Positions: {round(self.market_value, 2)}$")
    ...
    def add_position(self, symbol, nb, bought_price, avg_price, date):
        if nb > 0:
            ...
            self.refresh_market_value(date)
            print(f'{date} | BUY | {symbol} x{nb} | Capital: {round(self.capital, 2)}$ | Positions: {round(self.market_value, 2)}$')
